...
features_2012 <- data %>%
                  filter(year == 2012) %>%
                  group_by(contact_id) %>%
                  summarize(total_sales_2012 = sum(sales_amount, na.rm = TRUE),
                            total_line_items_2012 = n(),
                            total_purchases_2012 = n_distinct(order_id),
                            total_quantity_2012 = sum(quantity, na.rm = TRUE)
                            ) %>%
...
Making predictions on the zeros is one way to handle this issue - another one would be two build separate models for returning customers (a more complex one), and new customers.  
...
#base model
set.seed(93)
glm_model <- train(total_sales_2013 ~ total_sales_2012,
                   method = "glm",
                   data = data_train,

...
glm_pred <- bind_cols(predicted = glm_full_predictions, glm_just_returning_predictions) %>% as.data.table()
...
rmse_just_returning <- RMSE(data_test[existing_2012_contact_id == "Yes"]$total_sales_2013,
                            glm_pred[existing_2012_contact_id == "Yes"]$predicted)

rmse_corr_full <- RMSE(data_test$total_sales_2013,
                       glm_just_returning_predictions$predicted_sales_w_corr)
rmse_corr_just_returning <- RMSE(data_test[existing_2012_contact_id == "Yes"]$total_sales_2013,
                                 glm_just_returning_predictions[existing_2012_contact_id == "Yes"]$predicted_sales_w_corr)

```


```{r}
glm_test_pred <- bind_cols(actual = data_test$total_sales_2013, predicted = glm_predictions)
mean(df_for_prediction$total_sales_2013)
sd(df_for_prediction$total_sales_2013)

ggplot(data = glm_test_pred, aes(x=actual, y=predicted)) +
  geom_point()

##TODO:
 #- predict new customers just by the average in 2012
 #- visualize in two facets
 #- with 45 grid line signalling "good" predictions
 #- build function for this that can be used in all models
```
